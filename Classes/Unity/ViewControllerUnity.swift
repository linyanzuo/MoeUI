...
protocol ViewControllerUnity: Unity where Self: UIViewController  {
...
extension ViewControllerUnity {
    /// 统一编程规范: 导航配置的初始化
    /// ---
    /// 该方法应在`viewDidLoad`中被执行, 子类重写该方法, 完成控制器中"导航相关的初始化"工作
    ...
    /// 统一编程规范: 视图的初始化
    /// ---
    /// 该方法应在`viewDidLoad`中被执行, 子类重写该方法, 为控制器添加视图 理论上控制器只负责根视图的初始化工作, 建议由根视图类自行处理子视图的相关初始化
    ...
    /// 统一编程规范: 视图约束的初始化
    /// ---
    /// 该方法应在视图初始化完成后执行, 子类重写该方法, 为控制器中的视图添加约束. 理论上控制器只负责根视图的初始化工作, 建议由根视图类自行处理子视图的相关初始化
    func setupViewConstraints() {}
